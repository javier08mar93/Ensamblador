...
public class Linea {    
    ...
    String etiqueta, codop, operando, tipoError, instruccion, modo, modos, codMaquina, byteCalculado, byteporCalcular, totalBytes, contLoc, auxCont, valorOperando;
    ...
    public boolean ValidaToken(String linea, Lista lista, ModosDireccionamiento dir, Directivas direct, String auxC, boolean esORG, Vector<String> etiq) {
        ...
        if(codop != null && formatoCorrecto) {               
            if(!codop.matches("[a-zA-Z]((\\.[a-zA-Z]{0,3}|[a-zA-Z]{0,1}\\.[a-zA-Z]{0,2}|[a-zA-Z]{0,2}\\.[a-zA-Z]{0,1}|[a-zA-Z]{0,3}\\.)|[a-zA-Z]{0,4})")) {
                ...
                modo = codMaquina = "";
                if(esDirectivaValida(etiqueta, codop, operando, dir, direct, auxC, esORG)) {
                    esDirectiva = true;                
                                ...
                                if(etiqueta != null && formatoCorrecto) {
                                    ...
                                    if(formatoCorrecto && tipoError == null) {
                                        if(ValidaEtiq(etiqueta, etiq))
                                            ...
                                            if(modo.equals("INH") || modo.equals("DIR") || modo.equals("EXT") || modo.equals("IMM8") || modo.equals("IMM16")) {
                                               if(dir.valor >= 0 && (!operando.equals("NULL") || !operando.matches("[A-Za-z][\\w]{0,7}"))) {
                                                    if(!operando.matches("[A-Za-z][\\w]{0,7}")) {   
                                                    String valorHexa = Integer.toHexString(dir.valor);                                                
                                                    for (int i = valorHexa.length(); i < Integer.parseInt(byteporCalcular) * 2; i++)
                                                        valorOperando = valorOperando + "0";
                                                    valorOperando = valorOperando + Integer.toHexString(dir.valor).toUpperCase();
                                                    }
                                                }
                                                else if(dir.valor < 0) {
                                                    String auxOpe = "", ope = "";
                                                    ope = Integer.toHexString(dir.valor).toUpperCase();
                                                    int i = 0;
                                                    while(i < ope.length()){
                                                    if(ope.charAt(i) == 'F' && i+1 != ope.length() && ope.toUpperCase().charAt(i+1) == 'F') 
                                                        auxOpe = auxOpe;
                                                    else
                                                        auxOpe = auxOpe + ope.toUpperCase().charAt(i);
                                                    i++;
                                                    }
                                                    for (i = auxOpe.length(); i < Integer.parseInt(byteporCalcular) * 2; i++)
                                                        valorOperando = valorOperando + "0";
                                                    valorOperando = valorOperando + auxOpe.toUpperCase();                                                
                                                }
                                            }
                                        }                                        
                                    }                                    
    ...
    public void SeparaTABOP(String linea, Lista lista) {
     ...
     public boolean SeparaModos(String mod, String ope, ModosDireccionamiento dir) {
        ...
        Vector<String> codM = new Vector<String>();
        Vector<String> bytexC = new Vector<String>();        
        ...
        while(token.hasMoreTokens()) {
            ...
            codM.add(token.nextToken(" "));  
            bytexC.add(token.nextToken("|")); 
        }               
        ...
        while(x < tipoModo.size() && !encontrado && !dir.errorEtiqueta) {
            ...
            if(encontrado) {
                byteOperando = Integer.parseInt(byteC.get(x));
                codMaquina = codM.get(x);
                byteporCalcular = bytexC.get(x).substring(1, bytexC.get(x).length());
            }
     ...
     public boolean esDirectivaValida(String etiqueta, String codop, String operando, ModosDireccionamiento dir,  Directivas direct, String auxC, boolean esORG) {
                    ...
                    if(operando.toUpperCase().equals("NULL")) {
                        ...
                        tipoError = "La Directiva END no Puede Llevar Operando"; 
                                ...
                                if(!operando.toUpperCase().equals("NULL")) {
                                    ...
                                    else if(operando.charAt(0) == '"' && operando.charAt(operando.length()-1) == '"' && operando.substring(1, operando.length()).matches(".+")) {
         ...
         if(numLinea == 1)
         {
             etiq.add(etiqueta);
                    ...
                    while(et < etiq.size() && !etqRepetida) {
                        if(etiqueta.equals(etiq.get(et)) && !etiqueta.toUpperCase().equals("NULL")) {
                            etqRepetida = true;
                        }
                        else
                            et++;
