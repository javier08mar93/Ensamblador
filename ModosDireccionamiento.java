...
public class ModosDireccionamiento {
    ...
    public boolean EXT(String ope) {
        ...
        if(ope.charAt(0) == '@' || ope.charAt(0) == '$' || ope.charAt(0) == '%' || ope.matches("(-)?[0-9]+") || ope.matches("[A-Za-z0-9]+")) {         
            ...
            if(ope.charAt(0) == '@'  && ope.substring(1, ope.length()).matches("[0-7]+"))
                ...
                valor = Integer.parseInt(ope); 
            else if(ope.matches("[A-Za-z][\\w]{0,7}"))
                formaCorrecta = true;
            else if(!ope.matches("[A-Za-z][\\w]{0,7}") && (ope.charAt(0) != '@' && ope.charAt(0) != '$' && ope.charAt(0) != '%' && !ope.matches("(-)?[0-9]+"))) {
                errorEtiqueta = true;
    ...
    public boolean REL(String ope, String mod) {
        ...
        if(formaCorrecta && error == null) {
            if(ope.matches("[A-Za-z][\\w]{0,7}")  && mod.equals("REL8")) {
                tipoModo = "REL8";
                formaCorrecta = true;
            }
            else if(ope.matches("[A-Za-z][\\w]{0,7}")  && mod.equals("REL9")) {
                tipoModo = "REL9";
                formaCorrecta = true;
            }
            else if(ope.matches("[A-Za-z][\\w]{0,7}")  && mod.equals("REL16")) {
                tipoModo = "REL16";
                formaCorrecta = true;
            }
